			...
			<div class="w1660 m_t70 m_l30 ">
				...
				<div class="w1660  bgc_f1f3f7 c_384856">
					...
					<ul class="h40 p_t40 font14  p_l40">
						...
						<li class="f_l w140 text-center h40 l_h40 bgc_fff ping_1 cursor_"><a
							class="c_384856 display_block"
				...
				<div class=" ping_2">
					<div class="w1660 bgc_fff">
						<div class="w1290 font14">
							<form action="${ctx}/crms/appraise/badAssessment" method="post">
								<div class=" w1290 p_l70 h40 p_t40 l_h40">
									<div class="f_l">统计时间范围：</div>
									<input type="hidden" id="filtrateTime" name="filtrateTime"
										value="${filtrateTime }" />
									<div id="sx0" onclick="addFiltrateTime('最近7天');"
										class="cursor_ one_check_only w20 h20 border_d9dde5 b_radius5 f_l m_r15 m_t10 bgc_e0e6ef bgc_e0e6ef">
										<img class="cursor_ one_check_ display_none m_b10 w20 h20"
											src="${ctx}/crm/images/black_check.png" />
									</div>
									<div class="f_l m_r35 l_h20">最近7天</div>

									<div id="sx1" onclick="addFiltrateTime('最近15天');"
										class="cursor_ one_check_only w20 h20 border_d9dde5 b_radius5 f_l m_r15 m_t10 bgc_e0e6ef">
										<img class="cursor_ one_check_ display_none m_b10 w20 h20"
											src="${ctx}/crm/images/black_check.png" />
									</div>
									<div class="f_l m_r35 l_h20">最近15天</div>
									<div id="sx2" onclick="addFiltrateTime('最近一个月');"
										class="cursor_ one_check_only w20 h20 border_d9dde5 b_radius5 f_l m_r15 m_t10 bgc_e0e6ef">
										<img class="cursor_ one_check_ display_none m_b10 w20 h20"
											src="${ctx}/crm/images/black_check.png" />
									</div>
									<div class="f_l m_r15 l_h20">最近一个月</div>

									<div class="f_l position_relative">
										<!-- <input name="beginTime"  class="bgc_f4f6fa border0 w240 p_l10 h50 m_r10" type="text" id="tser07"  readonly onclick="$.jeDate('#tser07',{insTrigger:false,isTime:true,format:'YYYY-MM-DD '})"> -->
									...
									<div class="f_l position_relative">
										<!-- <input name="endTime" class="bgc_f4f6fa border0 w240 p_l10 h50 m_l10 m_r40" type="text" id="tser08"  readonly onclick="$.jeDate('#tser08',{insTrigger:false,isTime:true,format:'YYYY-MM-DD '})"> -->
										<input class="bgc_f4f6fa border0 w240 p_l10 h50 m_l10 m_r40"
											type="text" id="tser08" readonly
											onclick="$.jeDate('#tser08',{insTrigger:false,isTime:true,format:'YYYY-MM-DD '})"
											name="endTime"
									...
									<input id="sx3" onclick="addFiltrateTime('查询');"
										class="m_r45 w100 lh50 bgc_fff h50 b_radius5 tk border_00a0e9 c_00a0e9 text-center  f_l cursor_"
							...
							<div class="w810 h50 p_t40 p_l70">
								<div>
									<a
										href="${ctx}/crms/appraise/middleBadAssessment?filtrateTime=最近7天"
										class=" w100 h40 border_d9dde5 b_radius5  text-center f_l l_h40 c_8493a8 cursor_">
										中差评数</a>
								</div>
								<div>
									<a
										href="${ctx}/crms/appraise/middleAssessment?filtrateTime=最近7天"
										class=" w100 h40 border_d9dde5 b_radius5  text-center f_l l_h40 c_8493a8 cursor_">
										中评数</a>
								</div>
								<div>
									<a href="${ctx}/crms/appraise/badAssessment?filtrateTime=最近7天"
								...
								<div class="w1280 p_l70 p_t20" style="margin-bottom: 1vw;">
									<table class="text-center font14">
										...
										<tr class=" h55 bgc_f4f4f4 ">
											<td class="w280 bgc_f4f4f4"><c:if
													test="${ratedVo.neutral==null}">0</c:if> <c:if
													test="${ratedVo.neutral!=null}">${ratedVo.neutral}</c:if></td>
											<td class="w470 bgc_f4f4f4"><c:if
													test="${ratedVo.bad==null}">0</c:if> <c:if
													test="${ratedVo.bad!=null}">${ratedVo.bad}</c:if></td>
											<td class="w470 bgc_f4f4f4"><c:if
													test="${ratedVo.volume==null}">0</c:if> <c:if
													test="${ratedVo.volume!=null}">${ratedVo.volume}</c:if></td>
									...
									<table class="text-center font14 m_t20">
										...
										<c:if test="${pagination.list.size() !=0 }">
											<c:forEach items="${pagination.list}" var="ratedVoList">
												<tr class=" h55 bgc_fafafa ">
													<td class="w260 bgc_fafafa"><c:if
															test="${ratedVoList.created ==null}">
															<fmt:formatDate value="${now}" type="both"
																dateStyle="long" pattern="yyyy-MM-dd" />
														</c:if> <c:if test="${ratedVoList.created !=null}">
															<fmt:formatDate value='${ratedVoList.created}'
																pattern='yyyy-MM-dd' />
														</c:if></td>
													<td class="w310 bgc_f4f4f4"><c:if
															test="${ratedVoList.neutral == null}">0</c:if> <c:if
															test="${ratedVoList.neutral !=null}">${ratedVoList.neutral}</c:if>
													</td>
													<td class="w320 bgc_fafafa"><c:if
															test="${ratedVoList.bad ==null }">0</c:if> <c:if
															test="${ratedVoList.bad!=null }">	${ratedVoList.bad }</c:if>
													</td>
													<td class="w340 bgc_f4f4f4"><c:if
															test="${ratedVoList.volume ==null}">0</c:if> <c:if
															test="${ratedVoList.volume !=null}">${ratedVoList.volume}</c:if>
													</td>
...
<script>
	...
	function addFiltrateTime(data){
		...
		var filtrateTime = $('#filtrateTime').val();
		if(filtrateTime=='最近7天'){
			window.location.href="${ctx}/crms/appraise/badAssessment?filtrateTime=最近7天";
		}else if(filtrateTime=='最近15天'){
			window.location.href="${ctx}/crms/appraise/badAssessment?filtrateTime=最近15天";
		}else if(filtrateTime=='最近一个月'){
			window.location.href="${ctx}/crms/appraise/badAssessment?filtrateTime=最近一个月";
		}else{
		   window.location.href="${ctx}/crms/appraise/badAssessment?filtrateTime="+filtrateTime;
		}
