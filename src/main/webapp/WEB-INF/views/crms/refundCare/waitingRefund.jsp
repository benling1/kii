						...
						<div class="w1030">
							...
							<div class="change_1">
								<div class="w1000 font14 c_384856 h45 lh45 p_l30 m_b20">
									<div class="f_l m_r20">通知时间:</div>
									<div class="f_l m_r10 Time">${waitingRefund.startTime}</div>
									<div class="f_l m_r10">~</div>
									<div class="f_l m_r25 Time2">${waitingRefund.endTime}</div>
									<div class="f_l">超出时间次日通知</div>
								</div>

								<div class="font14 c_384856 h45 lh45 p_l30 m_b20">
									<div class="f_l m_r20">实付金额:</div>
									<div class="f_l Money">
										<c:if test="${waitingRefund.payAmtOne == 0} ">不限</c:if>
										<c:if test="${waitingRefund.payAmtOne !=0}">${waitingRefund.payAmtOne} </c:if>
									</div>
									<div class="f_l m_r10">~</div>
									<div class="f_l Money">
										<c:if test="${waitingRefund.payAmtTwo == 0}">不限</c:if>
										<c:if test="${waitingRefund.payAmtTwo != 0}">${waitingRefund.payAmtTwo}</c:if>
									</div>
								</div>
							</div>
							...
							<form action="${ctx}/waitingbasicSetup" method="post"
								id="formOne">
								<div class="display_none  change_2">
									<input name="id" value="${waitingRefund.id}" type="hidden" />
									<div class="w1000 font14 c_384856 h45 lh45 p_l30 m_b20">
											...
											<div class="wrap f_l h45 lh45 w140">
												<div class="nice-select h45 lh45 w117" name="nice-select">
													<input readonly="readonly" id="startTime"
														...
														style="color: #000000;" value="${waitingRefund.startTime}"
														style="color: #000000;">
													<ul>
														<li>00:00</li>
														<li>01:00</li>
											...
											<div class="wrap f_l h45 lh45 w140 m_l10">
												<div class="nice-select h45 lh45 w117" name="nice-select">
													<input readonly="readonly" id="endTime"
														...
														value="${waitingRefund.endTime }" style="color: #000000;">
													<ul>
														<li>00:00</li>
														<li>01:00</li>
														<li>02:00</li>
										...
										<div class="f_l row">
											<div class="m_l10 f_l font14 c_8493a8 lh45 col-lg-4"
									...
									<div class="f_r m_t10 p_l20">
										<div onclick="waitingRefund('formOne')"
						...
						<div class="w1030">
							...
							<div class="die_SZ m_b40 SaveG1">
								<div class="font14 c_384856 h50 lh50 p_l30">
									...
									<div class="f_l DiQu">
										<c:choose>
											<c:when test="${waitingRefundGj.locality==null }">
	                                        	 默认全部 
											    </c:when>
											<c:when test="${waitingRefundGj.locality=='0' }">
											   		 默认全部  
											    </c:when>
											<c:when test="${waitingRefundGj.locality=='上海市,浙江省,江苏省' }">
											   		 江浙沪 
											    </c:when>
											<c:when
												test="${waitingRefundGj.locality=='新疆维吾尔自治区,云南省,青海省,西藏自治区' }">
											   		偏远地区  
											    </c:when>
											<c:otherwise>  
											    	自定义
								...
								<div class="font14 c_384856 h50 lh50 p_l30">
									...
									<div class="f_l">
										<c:if test="${waitingRefundGj.vendormark==null }">
												所有卖家标记都不屏蔽
	                                        </c:if>
										<c:if test="${waitingRefundGj.vendormark=='0' }">
												所有卖家标记都不屏蔽
	                                        </c:if>
										<c:if test="${waitingRefundGj.vendormark=='1' }">
												屏蔽卖家标记
	                                        </c:if>
								...
								<div class="font14 c_384856 h50 lh50 p_l30">
									...
									<div class="f_l LaiYuan">
										<c:if test="${waitingRefundGj.orderSource==null }">
												不限
	                                        </c:if>

										<c:if test="${waitingRefundGj.orderSource=='0' }">
												不限
	                                        </c:if>
										<c:if test="${waitingRefundGj.orderSource=='1' }">
												PC端
	                                        </c:if>
										<c:if test="${waitingRefundGj.orderSource=='2' }">
												移动端
	                                        </c:if>
								...
								<div class="font14 c_384856 h50 lh50 p_l30">
									...
									<div class="f_l AllShangPin">
										<c:if test="${waitingRefundGj.productSelect==null }">
												全部商品
	                                        </c:if>
										<c:if test="${waitingRefundGj.productSelect=='0' }">
												全部商品
	                                        </c:if>
										<c:if test="${waitingRefundGj.productSelect=='1' }">
												指定商品
	                                        </c:if>
										<c:if test="${waitingRefundGj.productSelect=='2' }">
												排除指定商品
	                                        </c:if>
									</div>
								</div>
							...
							<div class="display_none Live_SZ SaveG2">
								...
								<div class="f_r m_t150 p_l20">
									<div onclick="submitFormAdvanced('advanced_form2')"
								...
								<!-- 保存按钮点击提交form表单 -->
								<form id="advanced_form2" action="${ctx}/waitingRefundGjSetup"
									method="post">
									<input type="hidden" name="id" value="${waitingRefundGj.id }" />
									<%-- 地区筛选 --详细地区--%>
									<input id="locality" type="hidden" name="locality"
										value="${waitingRefundGj.locality }" />
									<%-- 地区筛选 --详细地区--%>
									<input id="vendormark" type="hidden" name="vendormark"
										value="${waitingRefundGj.vendormark }" />
									<%-- 卖家标记--标识 --%>
									<input id="flagcolor" type="hidden" name="flagcolor"
										value="${waitingRefundGj.flagcolor }" />
									<%-- 卖家标记--屏蔽的卖家标记小旗颜色 --%>
									<input id="orderSource" type="hidden" name="orderSource"
										value="${waitingRefundGj.orderSource }" />
									<%-- 订单来源 --%>
									<input id="memberLevel" type="hidden" name="memberLevel"
										value="${waitingRefundGj.memberLevel }" />
									<%-- 会员等级 --%>
									<input id="productSelect" type="hidden" name="productSelect"
										value="${waitingRefundGj.productSelect }" />
									<%-- 商品选择--标识 --%>
									<input id="itemId" type="hidden" name="itemId"
										value="${waitingRefundGj.itemId }" />
									<%-- 商品选择--商品id --%>
								</form>
								<div style="clear: both"></div>
							</div>
						...
						<div class="w1030 p_b40">
							...
							<div class="die_SZ SaveS1">
								<div class="font14 c_384856 h40 lh40 p_l30">
									...
									<div class="f_l w910 h285 bgc_f4f6fa radius10">
										<div class="f_l w910 h285 bgc_f4f6fa radius10">
								...
								<div class="clear  h50 p_t40 w226  margin0_auto">
									<!-- <input type="button" name="" id="addstatus" value="开启同意退款，等待买家退货" class="w226 h50 lh50  b_radius5 bgc_00a0e9 c_fff text-center margin0_auto cursor_ font16 lijikaiqi border0"/> -->
									<input type="button" name="" id="addstatus"
										value="开启同意退款，等待买家退货"
										class="w226 h50 lh50  b_radius5 bgc_00a0e9 c_fff text-center margin0_auto cursor_ font16 lijikaiqi__2 border0" />
									<input id="buttonId" type="hidden"
										value="${waitingRefund.status }" /> <input id="orderSetupId"
										type="hidden" value="${waitingRefund.id }" />
							...
							<div class="Live_SZ display_none SaveS2">
								<form action="${ctx}/saveWaitingRefundSmsSetting" id="smsSubmit"
									method="post">
								...
								<div class="font14 c_384856 m_t30">
									...
									<div class="f_l w_900 h285 bgc_f4f6fa radius10">
										...
										<textarea id="textContent" name="content"
											class="w860 h240 outline_none radius10 border0 bgc_f4f6fa p_t15 p_r20 Letter11  text_area"
...
<script>
	...
	$(function() {
		$("#addstatus").click(function() {
			...
			if (buttonId == '0') {
				$("#buttonId").val('1');
				//$("#addstatus").val("开启同意退款，等待买家退货");
			} else if (buttonId == '1') {
				$("#buttonId").val('0');
				//$("#addstatus").val("关闭同意退款，等待买家退货");
			}
			addstatus();
		});
	});
	...
	$(function() {
		var buttonId = $("#buttonId").val();
		if (buttonId == '0') {
			$("#addstatus").val("已开启同意退款，等待买家退货");
		}
		if (buttonId == '1') {
			$("#addstatus").val("已关闭同意退款，等待买家退货");
		}
	});
