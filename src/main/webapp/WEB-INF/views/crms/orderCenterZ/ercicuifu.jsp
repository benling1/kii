...
<script type="text/javascript" src="${ctx}/crm/js/ercicuifu.js"></script>
			...
			<div class="w1660 m_t70 m_l30">
				...
				<div class="w1660  bgc_f1f3f7 c_384856">
					<div class=" p_l40 p_t27">
						<p class="font20" id="HEADER">二次催付</p>
					...
					<ul class="h40 m_t1_7 font14 p_l40">
						...
						<a href="${ctx}/crms/order/again" class="c_384856"><li
							class="f_l w140 text-center h40 l_h40 bgc_fff  cursor_ ">二次催付</li></a>
					...
					<div class="w1590  c_384856 p_l50 font16 p_t40">
						<div class="w1030">
							...
							<div class="die_SZ">
								<div
									...
									<div class="f_l m_r5 Time">
										<c:choose>
											<c:when test="${orderSetupAgain.startTime==null}">不限</c:when>
											<c:otherwise>${orderSetupAgain.startTime}</c:otherwise>
										</c:choose>
									</div>
									<div class="f_l m_r5">~</div>
									<div class="f_l m_r25 Time2">
										<c:choose>
											<c:when test="${orderSetupAgain.endTime==null}">不限</c:when>
											<c:otherwise>${orderSetupAgain.endTime}</c:otherwise>
										</c:choose>
									</div>
									<div class="f_l">
										<c:if test="${orderSetupAgain.filtingConditions==5}">超出时间不催付</c:if>
										<c:if test="${orderSetupAgain.filtingConditions==6}">超出时间次日催付</c:if>
									</div>
								</div>
								...
								<div class="font14 c_384856 h45 lh45 p_l30 m_b20">
									...
									<div id="minMoney" class="f_l Money m_r5">
										<c:choose>
											<c:when test="${orderSetupAgain.payAmtOne==null}">不限</c:when>
											<c:otherwise>${orderSetupAgain.payAmtOne}</c:otherwise>
										</c:choose>
									</div>
									<div class="f_l m_r5">~</div>
									<div id="maxMoney" class="f_l Money">
										<c:choose>
											<c:when test="${orderSetupAgain.payAmtTwo==null}">不限</c:when>
											<c:otherwise>${orderSetupAgain.payAmtTwo}</c:otherwise>
										</c:choose>
									</div>
								</div>
								...
								<div class="font14 c_384856 h45 lh45 p_l30 m_b20">
									...
									<div class="f_l Time3">第一次催付${orderSetupAgain.reminderTime}后，未付款</div>
								</div>
							</div>
							...
							<div class="display_none Live_SZ">
								<form id="changGuiForm" action="${ctx}/crms/order/again/setup"
									method="post">
									<div class="w1000 font14 c_384856 h45 lh45 p_l30 m_b20">
										<input id="filtInput" type="hidden" name="filtingConditions"
											value="${orderSetupAgain.filtingConditions}"> <input
											id="setupId" type="hidden" name="id"
											value="${orderSetupAgain.id}"> <input id="statusId"
											type="hidden" name="status" value="${orderSetupAgain.status}">
											...
											<div class="wrap f_l h45 lh45 w140">
												<div class="nice-select h45 lh45 w117 z_3"
													...
													<input readonly="readonly" id="startTime"
														...
														value="${orderSetupAgain.startTime}">
													<ul id="time01">
														<li>8:00</li>
														<li>9:00</li>
														<li>10:00</li>
										...
										<div class=" f_l h45 lh45">
											<div class="bgc_f4f6fa w140 f_l m_r10 radius10">
												<input
													...
													value="${orderSetupAgain.payAmtOne}" />
											...
											<div class="bgc_f4f6fa w140 f_l m_r10">
												<input
													...
													value="${orderSetupAgain.payAmtTwo}" />
												<div class="f_r w25">元</div>
											</div>
										</div>
									</div>
									...
									<div class="font14 c_384856  lh45 p_l30 m_b20 clearfix">
										...
										<div class="f_l">
										<div>
											<p style="color:#ff5751;">淘抢购二次催付时间加淘抢购常规催付时间不得大于15分钟。</p>
											<p style="color:#9fa3b5;">列如：淘抢购常规催付设置下单5分钟后发送催付短信，二次催付时间应设定为10（15-5）分钟内否则短信无法正常发送。</p>
										</div>
										<div class="clearfix">
										<div class="wrap f_l h45 lh45 m_r15">
											<div class="f_l m_r5 c_cbcdcf">第一次催付</div>
											<div class="wrap f_l h45 lh45 w140">
												<div class="nice-select h45 lh45 w117 z_2"
													...
													<input readonly="readonly" id="cuiFuId"
														...
														value="${orderSetupAgain.reminderTime}">
													<ul id="cuiFu00">
														<li>0分钟</li>
														<li>3分钟</li>
														<li>5分钟</li>
						...
						<div class="w1030">
							...
							<div class="die_SZ m_b40">
								<div class="font14 c_384856 h50 lh50 p_l30">
									...
									<div class="f_l DiQu">
										<c:choose>
											<c:when test="${orderAdvancedSettingAgain.locality==null }">
	                                        	默认全部

											    </c:when>
											<c:when test="${orderAdvancedSettingAgain.locality=='0' }">
											   		 默认全部  
											    </c:when>
											<c:when
												test="${orderAdvancedSettingAgain.locality=='上海市,浙江省,江苏省' }">
											   		 江浙沪 
											    </c:when>
											<c:when
												test="${orderAdvancedSettingAgain.locality=='新疆维吾尔自治区,云南省,青海省,西藏自治区' }">
											   		偏远地区  
											    </c:when>
											<c:otherwise>  
											    	自定义
								...
								<div class="font14 c_384856 h50 lh50 p_l30">
									...
									<div class="f_l">
										<c:if test="${orderAdvancedSettingAgain.vendormark==null }">
												所有卖家标记都不屏蔽
	                                        </c:if>
										<c:if test="${orderAdvancedSettingAgain.vendormark=='0' }">
												所有卖家标记都不屏蔽
	                                        </c:if>
										<c:if test="${orderAdvancedSettingAgain.vendormark=='1' }">
												屏蔽卖家标记
	                                        </c:if>
								...
								<div class="font14 c_384856 h50 lh50 p_l30">
									...
									<div class="f_l LaiYuan">
										<c:if test="${orderAdvancedSettingAgain.orderSource==null }">
												不限
	                                        </c:if>

										<c:if test="${orderAdvancedSettingAgain.orderSource=='0' }">
												不限
	                                        </c:if>
										<c:if test="${orderAdvancedSettingAgain.orderSource=='1' }">
												PC端
	                                        </c:if>
										<c:if test="${orderAdvancedSettingAgain.orderSource=='2' }">
												移动端
	                                        </c:if>
								...
								<div class="font14 c_384856 h50 lh50 p_l30">
									...
									<div class="f_l AllShangPin">
										<c:choose>
											<c:when
												test="${orderAdvancedSettingAgain.productSelect=='0'}">
                                        		全部商品
                                        	</c:when>
											<c:when
												test="${orderAdvancedSettingAgain.productSelect=='1'}">
                                        		指定商品
                                        	</c:when>
											<c:when
												test="${orderAdvancedSettingAgain.productSelect=='2'}">
                                        		排除指定商品
                                        	</c:when>
											<c:otherwise>
                                        		全部商品
							...
							<div class="display_none Live_SZ">
								...
								<form id="advanced_form"
									action="${ctx}/crms/order/again/advance" method="post">
									<input type="hidden" name="id"
										value="${orderAdvancedSettingAgain.id }" />
									<%-- 地区筛选 --详细地区--%>
									<input id="locality" type="hidden" name="locality"
										value="${orderAdvancedSettingAgain.locality }" />
									<%-- 地区筛选 --详细地区--%>
									<input id="vendormark" type="hidden" name="vendormark"
										value="${orderAdvancedSettingAgain.vendormark }" />
									<%-- 卖家标记--标识 --%>
									<input id="flagcolor" type="hidden" name="flagcolor"
										value="${orderAdvancedSettingAgain.flagcolor }" />
									<%-- 卖家标记--屏蔽的卖家标记小旗颜色 --%>
									<input id="orderSource" type="hidden" name="orderSource"
										value="${orderAdvancedSettingAgain.orderSource }" />
									<%-- 订单来源 --%>
									<input id="memberLevel" type="hidden" name="memberLevel"
										value="${orderAdvancedSettingAgain.memberLevel }" />
									<%-- 会员等级 --%>
									<input id="productSelect" type="hidden" name="productSelect"
										value="${orderAdvancedSettingAgain.productSelect }" />
									<%-- 商品选择--标识 --%>
									<input id="itemId" type="hidden" name="itemId"
										value="${orderAdvancedSettingAgain.itemId }" />
									<%-- 商品选择--商品id --%>
								</form>
							</div>
					...
					<div class="w1590  bgc_fff c_384856 p_l50 font16 DuanXinSheZhi">
						<div class="w1030 p_b40">
							...
							<div class="Live_SZ  min_h570 DXSZBianJiBuFen display_none">
								<form id="smsSubmit" action="${ctx}/crms/again/sms"
									method="post">
								...
								<div class="font14 c_384856 m_t30">
									...
									<div class="f_l w928 h285 bgc_f4f6fa radius10">
										<input type="hidden" id="settingType" name="settingType"
											value="3"> <input id="textContent1" type="hidden"
										...
										<div class="h30 m_l20 c_bfbfbf f_l">
											已输入：<span id="inputNum" class="text_count">1</span>字 当前计费：<span
								...
								<!-- <div onclick="" class="clear w226 h50 lh50  b_radius5 bgc_00a0e9 c_fff text-center margin0_auto m_t60 cursor_" >
                                    立即开启
                                </div> -->
							</div>
...
//	 		document.getElementById(fomrId).submit();
	...
	$(function(){
		...
		var productSelect = $("#productSelect").val();
			...
			if(status == '0'){
				$('#kaiQiId').val('已开启二次催付提醒');
			}else if(status == '1'){
				$('#kaiQiId').val('已关闭二次催付提醒');
			}else{
				$('#kaiQiId').val('开启二次催付提醒');
			}
...
// 		}
	...
	$('.lijikaiqi').click(function(){
		var status = $('#statusId').val();
		if(status==1){
			$.post("${ctx}/getOrderSetupInfo", {"type": "2"}, function (data) {
		        if (!data.sussce) {
		        	$(".tishi_2").show();
					$(".tishi_2").children("p").text("请先开启常规催付！")
					setTimeout(function(){ 
					$(".tishi_2").hide()
					},3000)  
		            return false;
