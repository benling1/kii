...
<script type="text/javascript" src="${ctx}/crm/js/juhusuan.js"></script>
			...
			<div class="w1660 m_t70 m_l30">
				...
				<div class="w1660  bgc_f1f3f7 c_384856">
					<div class=" p_l40 p_t27">
						<p class="font20" id="HEADER">聚划算催付</p>
					...
					<ul class="h40 m_t1_7 font14 p_l40">
						...
						<a href="${ctx}/crms/order/jhs" class="c_384856"><li
							class="f_l w140 text-center h40 l_h40 bgc_fff cursor_">聚划算催付</li></a>
					...
					<div class="w1590  c_384856 p_l50 font16 p_t40">
						<div class="w1030">
							...
							<div class="die_SZ">
								<div
									...
									<div class="f_l m_r5 Time">
										<c:choose>
											<c:when test="${orderSetupJhs.startTime==null}">不限</c:when>
											<c:otherwise>${orderSetupJhs.startTime}</c:otherwise>
										</c:choose>
									</div>
									<div class="f_l m_r5">~</div>
									<div class="f_l m_r25 Time2">
										<c:choose>
											<c:when test="${orderSetupJhs.endTime==null}">不限</c:when>
											<c:otherwise>${orderSetupJhs.endTime}</c:otherwise>
										</c:choose>
									</div>
									<div class="f_l">
										<c:if test="${orderSetupJhs.filtingConditions==5}">超出时间不催付</c:if>
										<c:if test="${orderSetupJhs.filtingConditions==6}">超出时间次日催付</c:if>
									</div>
								</div>
								...
								<div class="font14 c_384856 h45 lh45 p_l30 m_b20">
									...
									<div id="minMoney" class="f_l Money m_r5">
										<c:choose>
											<c:when test="${orderSetupJhs.payAmtOne==null}">不限</c:when>
											<c:otherwise>${orderSetupJhs.payAmtOne}</c:otherwise>
										</c:choose>
									</div>
									<div class="f_l m_r5">~</div>
									<div id="maxMoney" class="f_l Money">
										<c:choose>
											<c:when test="${orderSetupJhs.payAmtTwo==null}">不限</c:when>
											<c:otherwise>${orderSetupJhs.payAmtTwo}</c:otherwise>
										</c:choose>
									</div>
								</div>
								...
								<div class="font14 c_384856 h45 lh45 p_l30 m_b20">
									...
									<div class="f_l Time3">下单${orderSetupJhs.reminderTime}后，未付款</div>
								</div>
							</div>
							...
							<div class="display_none Live_SZ">
								<form id="changGuiForm" action="${ctx}/crms/order/jhs/setup"
									method="post">
									<div class="w1000 font14 c_384856 h45 lh45 p_l30 m_b20">
										<input id="filtInput" type="hidden" name="filtingConditions"
											value="${orderSetupJhs.filtingConditions}"> <input
											id="setupId" type="hidden" name="id"
											value="${orderSetupJhs.id}"> <input id="statusId"
											type="hidden" name="status" value="${orderSetupJhs.status}">
										...
										<div class=" f_l h45 lh45">
											<div class="bgc_f4f6fa w140 f_l m_r10 radius10">
												<input id="minPayment"
													...
													type="text" name="payAmtOne"
													value="${orderSetupJhs.payAmtOne}" />
											...
											<div class="bgc_f4f6fa w140 f_l m_r10">
												<input id="maxPayment"
													...
													type="text" name="payAmtTwo"
													value="${orderSetupJhs.payAmtTwo}" />
												<div class="f_r w25">元</div>
											</div>
										</div>
									</div>
											...
											<div class="wrap f_l h45 lh45 w140">
												<div class="nice-select h45 lh45 w117 z_2"
													...
													<input readonly="readonly" id="cuiFuId"
														...
														value="${orderSetupJhs.reminderTime}">
													<ul id="cuiFu00">
														<li>5分钟</li>
														<li>10分钟</li>
														<li>15分钟</li>
						...
						<div class="w1030">
							...
							<div class="die_SZ m_b40">
								<div class="font14 c_384856 h50 lh50 p_l30">
									...
									<div class="f_l DiQu">
										<c:choose>
											<c:when test="${orderAdvancedSettingJhs.locality==null }">
	                                        	默认全部

											    </c:when>
											<c:when test="${orderAdvancedSettingJhs.locality=='0' }">
											   		 默认全部  
											    </c:when>
											<c:when
												test="${orderAdvancedSettingJhs.locality=='上海市,浙江省,江苏省' }">
											   		 江浙沪 
											    </c:when>
											<c:when
												test="${orderAdvancedSettingJhs.locality=='新疆维吾尔自治区,云南省,青海省,西藏自治区' }">
											   		偏远地区  
											    </c:when>
											<c:otherwise>  
											    	自定义
								...
								<div class="font14 c_384856 h50 lh50 p_l30">
									...
									<div class="f_l">
										<c:if test="${orderAdvancedSettingJhs.vendormark==null}">
												所有卖家标记都不屏蔽
	                                        </c:if>
										<c:if test="${orderAdvancedSettingJhs.vendormark=='0'}">
												所有卖家标记都不屏蔽
	                                        </c:if>
										<c:if test="${orderAdvancedSettingJhs.vendormark=='1'}">
												屏蔽卖家标记
	                                        </c:if>
								...
								<div class="font14 c_384856 h50 lh50 p_l30">
									...
									<div class="f_l LaiYuan">
										<c:if test="${orderAdvancedSettingJhs.orderSource==null }">
												不限
	                                        </c:if>

										<c:if test="${orderAdvancedSettingJhs.orderSource=='0' }">
												不限
	                                        </c:if>
										<c:if test="${orderAdvancedSettingJhs.orderSource=='1' }">
												PC端
	                                        </c:if>
										<c:if test="${orderAdvancedSettingJhs.orderSource=='2' }">
												移动端
	                                        </c:if>
								...
								<div class="font14 c_384856 h50 lh50 p_l30">
									...
									<div class="f_l AllShangPin">
										<c:choose>
											<c:when test="${orderAdvancedSettingJhs.productSelect=='0'}">
	                                        		全部商品
	                                        	</c:when>
											<c:when test="${orderAdvancedSettingJhs.productSelect=='1'}">
	                                        		指定商品
	                                        	</c:when>
											<c:when test="${orderAdvancedSettingJhs.productSelect=='2'}">
	                                        		排除指定商品
	                                        	</c:when>
											<c:otherwise>
	                                        		全部商品
							...
							<div class="display_none Live_SZ">
								...
								<div style="clear: both"></div>
								<!-- 保存按钮点击提交form表单 -->
								<form id="advanced_form" action="${ctx}/crms/order/jhs/advance"
									method="post">
									<input type="hidden" name="id"
										value="${orderAdvancedSettingJhs.id }" />
									<%-- 地区筛选 --详细地区--%>
									<input id="locality" type="hidden" name="locality"
										value="${orderAdvancedSettingJhs.locality }" />
									<%-- 地区筛选 --详细地区--%>
									<input id="vendormark" type="hidden" name="vendormark"
										value="${orderAdvancedSettingJhs.vendormark }" />
									<%-- 卖家标记--标识 --%>
									<input id="flagcolor" type="hidden" name="flagcolor"
										value="${orderAdvancedSettingJhs.flagcolor }" />
									<%-- 卖家标记--屏蔽的卖家标记小旗颜色 --%>
									<input id="orderSource" type="hidden" name="orderSource"
										value="${orderAdvancedSettingJhs.orderSource }" />
									<%-- 订单来源 --%>
									<input id="memberLevel" type="hidden" name="memberLevel"
										value="${orderAdvancedSettingJhs.memberLevel }" />
									<%-- 会员等级 --%>
									<input id="productSelect" type="hidden" name="productSelect"
										value="${orderAdvancedSettingJhs.productSelect }" />
									<%-- 商品选择--标识 --%>
									<input id="itemId" type="hidden" name="itemId"
										value="${orderAdvancedSettingJhs.itemId }" />
									<%-- 商品选择--商品id --%>
								</form>
							</div>
					...
					<div class="w1590  bgc_fff c_384856 p_l50 font16 DuanXinSheZhi">
						<div class="w1030 p_b40">
							...
							<div class="Live_SZ  min_h570 DXSZBianJiBuFen display_none">
								<form id="smsSubmit" action="${ctx}/crms/jhs/sms" method="post">
								...
								<div class="font14 c_384856 m_t30">
									...
									<div class="f_l w928 h285 bgc_f4f6fa radius10">
										<input type="hidden" id="settingType" name="settingType"
											value="4s"> <input id="textContent1" type="hidden"
...
//	 		document.getElementById(fomrId).submit();
	...
	$(function(){
		...
		var productSelect = $("#productSelect").val();
			...
			if(status == '0'){
				$('#kaiQiId').val('已开启聚划算催付提醒');
			}else if(status == '1'){
				$('#kaiQiId').val('已关闭聚划算催付提醒');
			}else{
				$('#kaiQiId').val('开启聚划算催付提醒');
			}
