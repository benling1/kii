	...
	private void validateCity(TmcMessages tmcMessages,boolean proviceIsNull){
        ...
        if(ValidateUtil.isEmpty(sellerCity)){
            if(!proviceIsNull){
                ...
                this.logger.debug("地区省份筛选判断未通过且无城市设置，订单省份: "+tmcMessages.getTrade().getReceiverState()+" ,用户选择的筛选省份: "+tradeSetup.getProvince()+" tid:"+tmcMessages.getTid()+
        ...
        if(ValidateUtil.isEmpty(tradeCity)){
            tmcMessages.setFlag(false); 
            this.logger.debug("订单的城市获取不到，订单城市: "+tmcMessages.getTrade().getReceiverCity()+" ,订单区: "+tmcMessages.getTrade().getReceiverDistrict()+" tid:"+tmcMessages.getTid()+
                    ",类型："+tmcMessages.getTradeSetup().getType()+" 用户设置的ID:+"+tradeSetup.getId());
            return ;
        }
        ...
        if(!sellerCity.contains(tradeCity)){
            tmcMessages.setFlag(false); 
            this.logger.debug("地区城市筛选判断未通过，订单城市: "+tradeCity+" ,用户选择的筛选城市: "+sellerCity+" tid:"+tmcMessages.getTid()+
                    ",类型："+tmcMessages.getTradeSetup().getType()+" 用户设置的ID:+"+tradeSetup.getId());
            return ;
        }
	}
