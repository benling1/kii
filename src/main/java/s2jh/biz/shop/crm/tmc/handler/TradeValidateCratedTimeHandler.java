	...
	public void doHandle(@SuppressWarnings("rawtypes") Map map) throws HandlerException {
		...
		if(tradeSetup.getChosenTime()!=null){
			...
			if(chosenTime>createTime){
				...
				this.logger.debug("订单创建时间判断未通过，订单创建时间为："+tradeCreated+",用户开启任务的时间为："+tradeSetup.getChosenTime()+"  tid:"+tmcMessages.getTid()+",类型："+tmcMessages.getTradeSetup().getType()+" 用户设置的ID:+"+tradeSetup.getId());
				return ;
			}
		}
	}
