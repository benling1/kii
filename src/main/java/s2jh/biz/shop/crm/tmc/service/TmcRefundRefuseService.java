...
*/
@Service
public class TmcRefundRefuseService {
	@Resource(name="tradeRefundRefuseChain")
	private DefaultHandlerChain tradeRefundRefuseChain;
	...
	private Logger logger = org.slf4j.LoggerFactory.getLogger(TmcRefundRefuseService.class);
	...
	public void doHandle(JSONObject content) throws Exception{
		...
		Map<Object,Object> tradeCreateSetupMaps = this.cacheService.hGetAll(OrderSettingInfo.TRADE_SETUP+sellerNick+"_"+OrderSettingInfo.REFUND_REFUSE); 
		List<TradeSetup> list = TmcDistributeService.sortTradeSetup(tradeCreateSetupMaps.values());
		if(ValidateUtil.isEmpty(list)){
			this.logger.debug("拒绝退款关怀未开启, " + content);
			return ;
		}
		...
		tmcMessages.setSettingType(OrderSettingInfo.REFUND_REFUSE);
		tmcMessages.setSendSchedule(false);
		Map<String,Object> map = new HashMap<String,Object>(5);
		map.put("tmcMessages", tmcMessages);
		this.logger.debug("拒绝退款开始处理  " + tmcMessages.getTid()+" ，用户设置了"+list.size()+"个任务");
		for (TradeSetup tradeSetup : list) {
			try {
				...
				this.tradeRefundRefuseChain.doHandle(map);
				this.logger.debug("拒绝退款流程处理完，tid: " + tmcMessages.getTid()+ "处理结果:"+tmcMessages.getFlag()+" ,id:"+tmcMessages.getTradeSetup().getId());
