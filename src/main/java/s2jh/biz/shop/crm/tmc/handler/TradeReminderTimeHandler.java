...
public class TradeReminderTimeHandler implements Handler {
	...
	public void doHandle(@SuppressWarnings("rawtypes") Map map) throws HandlerException {
		...
		if(remindTime==null){
			this.logger.debug("延后设置异常，用户的类型:"+timeType+",延后单位: "+remindTime+" tid:"+tmcMessages.getTid()+",类型："+tmcMessages.getTradeSetup().getType()+" 用户设置的ID:+"+tradeSetup.getId());
			return ;
		}
		this.logger.debug("延后之前的时间是:"+tmcMessages.getSendTime()+" tid:"+tmcMessages.getTid()+",类型："+tmcMessages.getTradeSetup().getType()+" 用户设置的ID:+"+tradeSetup.getId());
		...
		this.logger.debug("延后好的时间是:"+tmcMessages.getSendTime()+" tid:"+tmcMessages.getTid()+",类型："+tmcMessages.getTradeSetup().getType()+" 用户设置的ID:+"+tradeSetup.getId());
	}
