...
@Controller
@RequestMapping(value="paymentCare")
public class PaymentCareController {
	@Autowired
	 ...
	 * 查询基本设置和高级信息
	 */
	@RequestMapping(value = "/queryPaymentCare" ,method=RequestMethod.GET)
	public String queryOrderSetupAndOrderAdvancedSetting(String msg,HttpServletRequest request, ModelMap model){
		...
		if(userId!=null){
			...
			//放入域对象
			model.addAttribute("orderSetupPayment", orderSetupXDGH);
			//放入域对象
			model.addAttribute("orderAdvancedSettingPayment", orderAdvancedSetting);
		...
		return "crms/orderCenterH/paymentCare";
	}
	 ...
	 * @return
	 */
	@RequestMapping(value="/saveOrderSetupPaymentCare" ,method=RequestMethod.POST)
	public String saveOrUpdateOrderSetup(ModelMap model,OrderSetup orderSetup,HttpServletRequest request,
		...
		if(userId!=null){
			...
			if(orderSetupID != null && !"".equals(orderSetupID)){
				...
				try {
					 ...
					 */
					//操作日志数据补全
					op.setFunctions("付款关怀基本设置");
					op.setType("修改");
					op.setDate(new Date());
					op.setRemark("付款关怀基本设置");
					op.setIpAdd(getIpAddress.getIpAddress(request));
					op.setUserId(userId);
					op.setFunctionGens("13");
					//调用日志方法添加日志
		...
		return "redirect:queryPaymentCare";
	}
	
	/**
	 * 添加或修改高级设置======================================= 
	 */
	@RequestMapping(value="/saveOrderAdvancedSettingPaymentCare" ,method=RequestMethod.POST)
		...
		if(userId!=null){
			...
			if(userOrderAdvancedID != null && !"".equals(userOrderAdvancedID)){
				try {
					 ...
					 */
					//操作日志数据补全
					op.setFunctions("付款关怀高级设置");
					op.setType("修改");
					op.setDate(new Date());
					op.setRemark("付款关怀高级设置");
					op.setIpAdd(getIpAddress.getIpAddress(request));
					op.setUserId(userId);
					op.setFunctionGens("13");
					//调用日志方法添加日志
				...
				orderAdvancedSetting.setUserId(userId);
				//3.设置类型
				orderAdvancedSetting.setSettingType("13");
				//4.默认设置定时发送为关闭
		...
		if(userId!=null){
			...
			if(smsSettingID != null && !"".equals(smsSettingID)){
				try {
					 ...
					 */
					//操作日志数据补全
					op.setFunctions("付款关怀短信设置");
					op.setType("修改");
					op.setDate(new Date());
					op.setRemark("付款关怀短信设置");
					op.setIpAdd(getIpAddress.getIpAddress(request));
					op.setUserId(userId);
					op.setFunctionGens("13");
					//调用日志方法添加日志
