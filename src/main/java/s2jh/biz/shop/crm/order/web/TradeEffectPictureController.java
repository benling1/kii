...
/**
 * 订单短信群发效果分析Controller
 * @author Administrator
 *
 */
@Controller
@RequestMapping("/order")
public class TradeEffectPictureController {
...
//	private Map<String,Object> map = new HashMap<String, Object>();
	...
	/**
	 * 初次进入订单短信群发效果分析页面，只进行参数的保存和传递，不进行查询
...
//		map.put(userId + "endTime", endTime);
		...
		return "/crms/marketingCenter/orderEffectPicture";
	}
...
//	@RequestMapping("/customerIndex")
	...
	public String numberCustomerIndex(Model model){
		model.addAttribute("message", 1);
		return "/crms/marketingCenter/orderCustomerDetail";
	}
...
//	@RequestMapping("/itemIndex")
	...
	public String numberItemIndex(Model model){
		model.addAttribute("message", 1);
		return "/crms/marketingCenter/orderItemDetail";
	}
	...
	public String tradeMsgRecord(HttpServletRequest request,String activityName,String bTime,String eTime,Model model,
		...
		String contextPath = request.getContextPath()+"/order/msgSendRecord";
		...
		String url = request.getContextPath() + "/order/msgSendRecord";
		...
		return "crms/marketingCenter/orderMsgRecord";
	}
...
//			map.put(userId + "endTime", endTime);
			...
			if(endLongTime < todayLongTime){//效果分析结束时间小于现在时间，不需要计算实时的数据
				logger.info("订单短信群发的效果分析结束时间小于现在时间，不需要计算实时的数据,days:"+dayNum);
			}else {
				logger.info("订单短信群发的效果分析结束时间大于现在时间，需要计算实时的数据,days:"+dayNum);
				//效果分析结束时间大于现在时间，需要计算实时的数据
				//付款的订单状态的list
				List<String> payStatusList = new ArrayList<String>();
				payStatusList.add(TradeStatusUtils.TRADE_FINISHED);payStatusList.add(TradeStatusUtils.WAIT_SELLER_SEND_GOODS);payStatusList.add(TradeStatusUtils.TRADE_CLOSED);
				payStatusList.add(TradeStatusUtils.WAIT_BUYER_CONFIRM_GOODS);payStatusList.add(TradeStatusUtils.TRADE_BUYER_SIGNED);
...
//		Date endTime = (Date) map.get(userId + "endTime");
		...
		String url = request.getContextPath() + "/order/customerIndex";
		StringBuilder params = new StringBuilder();
...
//		Date beginTime = (Date) map.get(userId + "beginTime");
		...
		String url = request.getContextPath() + "/order/itemIndex";
		StringBuilder params = new StringBuilder();
		if(customerType != null && !"".equals(customerType)){
			params.append("&customerType=").append(customerType);
		}
