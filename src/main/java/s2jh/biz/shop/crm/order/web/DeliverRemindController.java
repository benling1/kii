...
/**
 * @ClassName: DeliverRemindController
 * @Description: TODO(物流提醒>>派件提醒)
 * @author:jackstraw_yu
 * @date 2016年12月17日
 *
 */
@Controller
@RequestMapping(value = "/deliverRemind")
public class DeliverRemindController {
	 ...
	 * @Description: TODO(物流提醒>>派件提醒页面跳转与查询全部)
	 * @param @param model
	 * @param @return 参数
	 * @return String 返回类型
	 * @author:jackstraw_yu
	...
	public String queryAllSettings(String msg, Model model,
		...
		if (userId != null) {
			...
			orderAdvancedSetting.setSettingType("8");
			orderAdvancedSetting.setUserId(userId);

			orderAdvancedSetting = orderAdvancedSettingWLTXService
					.queryOrderAdvancedSetting(orderAdvancedSetting);
			model.addAttribute("orderAdvancedSetting", orderAdvancedSetting);

			// 短信设置
			SmsSetting smsSetting = smsSettingService.findSmsSetting(userId,
					"8");
			model.addAttribute("smsSetting", smsSetting);
			if (orderSetup != null && "0".equals(orderSetup.getStatus())) {
				// 开启
				model.addAttribute("show", 0);
			} else if (orderSetup != null
					&& ("1".equals(orderSetup.getStatus()) || orderSetup
							.getStatus() == null)) {
				// 关闭
			...
			model.addAttribute("msg",
					ResourceGetValueUtil.getValue("user.account.time.out.msg"));
		}
		return "crms/order/deliverRemind";
	}

	/**
	 * @Title: saveOrderSetup
	 * @Description: TODO(订单中心>>派件提醒保存基本设置)
	 * @param @return 参数
	 * @return String 返回类型
	 * @author:jackstraw_yu
	 * @throws
	...
	public String saveOrderSetup(
		...
		if (userId != null) {
			...
			model.addAttribute(
					"msg",
					flag ? ResourceGetValueUtil
							.getValue("order.setup.save.success.msg")
							: ResourceGetValueUtil
									.getValue("order.setup.save.failure.msg"));
		} else {
			model.addAttribute("msg",
					ResourceGetValueUtil.getValue("user.account.time.out.msg"));
		}
		// 重定向到查询链接
		return "redirect:/deliverRemind/queryAllSettings";
	}

	/**
	 * @Title: saveAdvancedSetting
	 * @Description: TODO(派件提醒>>修改高级设置)
	...
	public void saveAdvancedSetting(OrderAdvancedSetting orderAdvancedSetting,
		...
		if (userId != null) {
			/*
			 * String locality,String vendormark,String orderSource,String
			 * memberLevel,String productSelect,String ids 地区筛选, 卖家标记, 订单来源,
			 * 会员等级, 排除/指定商品 商品id *
			 */
			orderAdvancedSetting.setSettingType("8");
			orderAdvancedSetting.setUserId(userId);
			...
			try {
				// service层添加或修改高级设置
				orderAdvancedSettingWLTXService.saveOrderAdvancedSetting(
						orderAdvancedSetting, op);
				js.put("message", true);
			} catch (Exception e) {
	...
	public void saveSmsSetting(SmsSetting smsSetting,
			HttpServletRequest request, HttpServletResponse response) {
	...
	public void updateStatus(String status, HttpServletRequest request,
