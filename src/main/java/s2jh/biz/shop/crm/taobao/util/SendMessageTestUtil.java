...
import s2jh.biz.shop.utils.MD5Utils;
import s2jh.biz.shop.utils.MsgUtil;
 ...
 */
public class SendMessageTestUtil {
	
	private static Logger logger = LoggerFactory.getLogger(SendMessageTestUtil.class);

//	private final static String YXCS_USERCODE = "yxzxcs";
//	private final static String YXCS_PASSWORD = "mpbg44zp";
	
	private final static String YXCS_USERCODE = "keyuncs";
	private final static String YXCS_PASSWORD = "ceshi";
	...
	private static Map<String, Object> buildRequestPara(String[] phones,String content,String ext,String reference) {
		...
		sendMap.put("name",YXCS_USERCODE);// 帐号
		sendMap.put("seed",orderNum);//当前时间，格式：YYYYMMDD HHMISS，例如：20130806102030。客户时间早于或晚于网关时间超过30分钟，则网关拒绝提交。
		sendMap.put("key",MD5Utils.sign(MD5Utils.sign(YXCS_PASSWORD,"","UTF-8").toLowerCase()+orderNum,"","UTF-8").toLowerCase());//签名
