...
public class RedisLock {
    ...
    private boolean expire(final String key,final long expireTime){
    	Object obj = null;
    	try {
    		obj = redisTemplate.execute(new RedisCallback<Object>() {
    			 @Override
                 public Object doInRedis(RedisConnection connection) throws DataAccessException {
    				 StringRedisSerializer serializer = new StringRedisSerializer();
                     Boolean success = connection.expire(serializer.serialize(key), expireTime);
                     connection.close();
    				 return success;
    			 }
    		});
    ...
    public boolean lock() throws InterruptedException {
        ...
        while (timeout >= 0) {
            ...
            if (this.setNX(lockKey, expiresStr)) {
                // lock acquired
            	this.expire(lockKey, timeoutMsecs*2 );
                locked = true;
                return true;
            }
        ...
        this.logger.debug("锁等待超时，警告！！！"+this.lockKey);
//        return false;
        throw new RuntimeException("锁等待超时！");
    }
