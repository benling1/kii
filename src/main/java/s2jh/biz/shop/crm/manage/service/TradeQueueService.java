...
public class TradeQueueService {
		...
		public  static   BlockingQueue<List<TbTransactionOrder>> queue = new ArrayBlockingQueue<List<TbTransactionOrder>>(GetTaobaoMSG.size);
		
		@Autowired
		private SynchronousTradeHelper  synchronousTradeHelper;  
		
		private static final Log logger = LogFactory.getLog(TradeQueueService.class);
	
		//回调处理链条
	    interface DataHandler{
	    	void doHandler(List<TbTransactionOrder> messageList);
	    }
	        ...
	        public TradeDataProcessor() {
	            ...
	            Runnable worker = new Runnable() {
	                public void run() {
	                	for(;;){
		                    try {
		                    	List<TbTransactionOrder> rsps  =  queue.take();
	                    		if(null!=rsps&&rsps.size()>0){
	                    			System.out.println("从Trade队列取走一个元素，队列剩余"+(GetTaobaoMSG.size-queue.size())+"个元素");
	                    			synchronousTradeHelper.loadConvertTbTradeData(rsps);
	                    		}else{
